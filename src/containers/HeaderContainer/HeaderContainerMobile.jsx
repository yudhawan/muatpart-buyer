...
function HeaderContainerMobile() {
  ...
  return (
    <header className={style.main} ref={headerRef}>
      {
        appBarType.includes('Category')&&<HeaderCategoryMobile/>
      }
      {
        appBarType.includes('Compact')&&<HeaderMainCompact/>
      }
      {
        !!(appBarType==='header_title_secondary'|appBarType==='header_search_secondary'|appBarType==='header_title'|appBarType==='header_search')&&<HeaderTitleSearchMobile appBar={appBar} type={appBarType} title={appBar?.title} onBack={handleBack} setSearch={setSearch} searchPlaceholder={search?.placeholder} searchValue={search?.value} />
      }
      {
        appBarType.includes('Modal')&&<HeaderModalMobile shadow={shadow} handleAction={handleAction} setAppBar={setAppBar} appBar={appBar} type={appBarType} title={appBar?.title} onBack={handleBack} setSearch={setSearch} searchPlaceholder={search?.placeholder} searchValue={search?.value} showReset={showReset} />
      }
      {
  ...
  return(
    <div className={`${isBgSecondary?'bg-neutral-50':'bg-[#c22716]'} relative w-full h-auto max-h-[88px] p-4 pb-3 flex gap-2 ${shadow? 'shadow-lg':''}`}>
      ...
      <div className='flex gap-5 w-full items-center'>
        ...
        {(type==='header_title' || type==='header_title_secondary')&&<span className={`font-bold text-base ${isBgSecondary?'text-[#176cf7]':'text-neutral-50'}`}>{title}</span>}
        {
...
export function HeaderModalMobile({appBar,handleAction,type,title,onBack,searchPlaceholder, searchValue,setSearch,shadow,showReset}) {
  const RenderBack=appBar?.renderBack||null
  const ActionButton = appBar?.renderActionButton||null
    ...
    <div className={`${type.includes('Secondary')?'bg-neutral-50':'bg-[#c22716]'} relative w-full h-auto max-h-[88px] p-4 pb-3 ${shadow? 'shadow-lg':''}`}>
        <div className='flex flex-col'>
          <div className='flex items-center justify-between gap-2'>
            ...
            {
              ActionButton?ActionButton:showReset?<span onClick={handleAction} className={`font-semibold text-sm cursor-pointer ${type.includes('Secondary')?'text-primary-700':'text-neutral-50'}`}>Reset</span>:''
            }
          </div>
          {!type.includes('Secondary')&&<Image src='/img/fallinstartheader.png' width={153} height={62} alt='fallin' className='absolute right-0 bottom-0' />}
        </div>
      </div>
  )
}

export function HeaderCategoryMobile({appBar,handleAction,type,title,onBack,searchPlaceholder, searchValue,setSearch,shadow,showReset}) {
  return(
    <div className={`${type.includes('Secondary')?'bg-neutral-50':'bg-[#c22716]'} relative w-full h-auto max-h-[88px] p-4 pb-3 ${shadow? 'shadow-lg':''}`}>
        <div className='flex flex-col'>
          <div className='flex items-center justify-between gap-2'>
