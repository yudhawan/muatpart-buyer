...
const Dropdown = ({ id, options: pilihan = [], optionsOther: pilihanLain = [], optionsOtherText = "", classname, onSearchValue, onSelected = () => {}, selectedIconElement, leftIconElement, placeholder = "Select value", searchPlaceholder = "Search...", showDropdown = false, isMultipleSelected = false, onCustom, textCustom, defaultValue, dateStartEnd, defaultShow = "", fixedPlaceholder }) => {
     ...
     const searchBy = search ? [...options, ...optionsOther]?.filter((val) => val.name?.toLowerCase().includes(search.toLowerCase())) : options?.filter((val) => val.name?.toLowerCase().includes(search.toLowerCase()));
     useEffect(() => {
               ...
               <ul className={style.listOptions}>
                    {searchBy.map((val, index) => (
                         <li
                              ...
                              onClick={(e) => {
                                   if (!isMultipleSelected) {
                                        val.value == "tanggal" ? setDatepicker(true) : handleSelect({ value: val.value, name: val.name, title: val.title || "" });
                                   } else {
                                        e.preventDefault();
                         ...
                         >
                              ...
                              {!!selected.filter((a) => a.value === val.value).length && !isMultipleSelected && <IconComponent src={"/icons/check-circle.svg"} />}
                         </li>
                    ))}
     ...
     return (
          <>
               <div ref={dropdownRef} className={`${style.main} ${classname}`}>
                    <button id={id} onClick={handleToggle} className={`${style.buttonPlace} ${!selected.length && "!text-neutral-600"} select-none`}>
                         <div className="flex gap-2 items-center">
                              ...
                              {isMultipleSelected && selected.length && !fixedPlaceholder > 1 ? (
                                   ...
                                   <span className="text-[12px] text-neutral-600 font-semibold">{`${formatDateFE(dateRange[0])} - ${formatDateFE(dateRange[1])}`}</span>
                              ) : (
                         ...
                         <div className={style.listContainer}>
                              {onSearchValue && (
                                   <div className="h-[32px] border border-neutral-500 rounded flex items-center py-2 px-3 ">
                                        ...
                                        <input type="text" value={search} onChange={handleSearch} placeholder={searchPlaceholder} className={`w-full h-full outline-none focus:outline-none rounded py-2 px-3 text-[12px] flex-1 text-neutral-900 placeholder:text-neutral-700`} />
                                        {search && <IconComponent src={"/icons/silang.svg"} onclick={(e) => setSearch("")} />}
                                   </div>
                              )}
                              {onCustom && (
                                   <div onClick={onCustom} className="flex cursor-pointer w-full justify-start items-center gap-2 border-b border-neutral-400 pb-3">
                                        <IconComponent src={"/icons/Plus.svg"} width={14} height={14} classname={style.customIcon} />
