...
function ModalComponent({children,full,hideHeader=false,headerSize='small',isOpen,setClose,preventAreaClose,showButtonClose=true,classname}) {
  ...
  if(getIsOpen) return (
    <div 
      ...
      className={`${style.main}  w-full overflow-hidden z-[92] flex justify-center items-center`}
      onClick={()=>{
        if(!preventAreaClose){
          setIsOpen(false)
          setClose?.()
    ...
    >
        ...
        {children?<div className={`bg-white rounded-[10px] p-2 z-40 relative min-w-[386px] min-h-[208px] overflow-hidden ${!hideHeader?'pt-[78px]':''} ${classname}`} onClick={e=>e.stopPropagation()}>
